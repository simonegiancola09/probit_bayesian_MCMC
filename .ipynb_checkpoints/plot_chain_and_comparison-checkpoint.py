...
def plot_double_dim_accepted_rejected(thetas, dimensions = (0,1), plot_theta_true = True,
    ...
    if plot_rejected:
        plt.plot(rejected[np.where(chain_rejected==True)][:1000,dim1], rejected[np.where(chain_rejected==True)][:1000,dim2], 'rx', label='Rejected', alpha=.4)
    if plot_theta_true:
...
def plot_triple_dim_accepted_rejected(thetas, dimensions = (0,1,2), plot_theta_true = True,
    ...
    ax1.plot(thetas[:1000,dim1], thetas[:1000,dim2], thetas[:1000,dim3], alpha=.7)
    ax1.plot(thetas[np.where(chain_rejected==False)][:500,dim1],
             thetas[np.where(chain_rejected==False)][:500,dim2],
             thetas[np.where(chain_rejected==False)][:500,dim3],
             'b.', label='Accepted', alpha=.5)
    if plot_rejected:
        ax1.plot(rejected[np.where(chain_rejected==True)][:300,dim1],
              rejected[np.where(chain_rejected==True)][:300,dim2],
               rejected[np.where(chain_rejected==True)][:300,dim3],
